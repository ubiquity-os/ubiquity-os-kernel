...
export type TelegramIdentityLookupResult =
  | {
      ok: true;
...
export type TelegramLinkedIdentityListResult =
  | {
      ok: true;
      identities: TelegramLinkedIdentityEntry[];
...
export type TelegramLinkIssueResult =
  | {
      ok: true;
      issue: TelegramLinkIssueRecord | null;
    }
  | { ok: false; error: string };

export type TelegramLinkCodePeekResult =
  | {
      ok: true;
      userId: number;
      expiresAtMs: number;
...
export type TelegramLinkPendingResult =
  | {
      ok: true;
      pending: TelegramLinkPendingState | null;
    }
  | { ok: false; error: string };

export type TelegramLinkIssueIndexResult =
...
function parseTelegramIdentityRecord(value: unknown): TelegramLinkedIdentity | null {
  ...
  const githubLogin = normalizeOptionalString(value.githubLogin)?.toLowerCase();
...
export async function getTelegramLinkCodeForIssue(params: {
  ...
  if (!kv) {
    return {
      ok: false,
      error: "KV unavailable; cannot read link issue index.",
    };
  }
...
export async function getTelegramLinkPending(params: { userId: number; logger: LoggerLike }): Promise<TelegramLinkPendingResult> {
  ...
  if (!kv) {
    return {
      ok: false,
      error: "KV unavailable; cannot read pending link state.",
    };
  }
...
export async function saveTelegramLinkPending(params: {
  ...
  if (!kv) {
    return {
      ok: false,
      error: "KV unavailable; cannot persist pending link state.",
    };
  }
  const key = buildTelegramLinkPendingKey(params.userId);
...
export async function clearTelegramLinkPending(params: { userId: number; logger: LoggerLike }): Promise<{ ok: true } | { ok: false; error: string }> {
  ...
  if (!kv || typeof kv.delete !== "function") {
    return {
      ok: false,
      error: "KV unavailable; cannot clear pending link state.",
    };
  }
  const key = buildTelegramLinkPendingKey(params.userId);
  try {
...
export async function saveTelegramLinkIssue(params: {
  ...
  try {
    ...
    await kv.set(
      ...
      {
        code: params.code,
        createdAtMs: params.issue.createdAtMs,
      },
      { expireIn: ttlMs }
    );
    return { ok: true };
...
export async function saveTelegramLinkedIdentity(params: {
  ...
  const normalizedGithubLogin = normalizeOptionalString(params.githubLogin)?.toLowerCase();
  try {
    if (params.ownerType === "user") {
      const existingOwners = await listOwnerTelegramUserIds({
        kv,
        owner: params.owner,
        limit: 2,
      });
      ...
      if (hasOtherUser) {
        return {
          ok: false,
          error: "This GitHub user is already linked to another Telegram account.",
        };
      }
    }
    ...
    const githubLoginToPersist = normalizedGithubLogin ?? existingRecord?.githubLogin;
    await kv.set(key, {
      owner: params.owner,
      ...(githubLoginToPersist ? { githubLogin: githubLoginToPersist } : {}),
      linkedAt,
    });
    await kv.set(buildOwnerTelegramKey(params.owner, params.userId), {

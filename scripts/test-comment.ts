...
async function processCommentWithRealPlugins(org: string, repo: string, commentBody: string, issueNumber = 1) {
...
function parseGitHubUrl(url: string): { org: string; repo: string; issueNumber: number } | null {
  try {
    // Handle both https://github.com/org/repo/issues/number and org/repo/issues/number formats
    ...
    // Also support just org/repo/issues/number format
    const shortPattern = /^([^/]+)\/([^/]+)\/issues\/(\d+)$/;
...
async function main() {
  ...
  if (args.length < 3) {
    console.log("Usage: bun run scripts/test-comment.ts <org> <repo> <comment-body> [github-url]");
    console.log("Examples:");
    console.log("  bun run scripts/test-comment.ts 0x4007 ubiquity-os-sandbox '/hello'");
    console.log("  bun run scripts/test-comment.ts 0x4007 ubiquity-os-sandbox '/hello' https://github.com/0x4007/ubiquity-os-sandbox/issues/2");
    console.log("  bun run scripts/test-comment.ts 0x4007 ubiquity-os-sandbox '/help' 0x4007/ubiquity-os-sandbox/issues/5");
    console.log("  bun run scripts/test-comment.ts 0x4007 ubiquity-os-sandbox '@UbiquityOS what commands are available'");
    process.exit(1);
  }

  const [org, repo, ...remainingArgs] = args;

  // Check if last arg is a GitHub URL
  let commentBody: string;
  let _githubUrl: string | undefined;
  let targetOrg = org;
  let targetRepo = repo;
  let issueNumber = 1; // Default fallback

  if (remainingArgs.length > 1) {
    // Check if the last argument looks like a GitHub URL
    const lastArg = remainingArgs[remainingArgs.length - 1];
    const parsedUrl = parseGitHubUrl(lastArg);

    if (parsedUrl) {
      _githubUrl = lastArg;
      commentBody = remainingArgs.slice(0, -1).join(" ");
      targetOrg = parsedUrl.org;
      targetRepo = parsedUrl.repo;
      issueNumber = parsedUrl.issueNumber;
      console.log(`🎯 Targeting issue #${issueNumber} in ${targetOrg}/${targetRepo}`);
    } else {
      commentBody = remainingArgs.join(" ");
    }
  } else {
    commentBody = remainingArgs.join(" ");
  }

  await processCommentWithRealPlugins(targetOrg, targetRepo, commentBody, issueNumber);
}
